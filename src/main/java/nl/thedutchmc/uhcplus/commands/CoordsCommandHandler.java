	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		for(Team team : TeamHandler.getAliveTeams()) {
			if(team.getAliveTeamMembers().contains(p.getUniqueId())) {
				...
				String message = ChatColor.AQUA + "[Team] " + ChatColor.GOLD + "Coordinates from Team member " + ChatColor.RED + p.getName() + 
						ChatColor.GOLD + ": X: " + ChatColor.RED + p.getLocation().getX() + ChatColor.GOLD + " Y: " + ChatColor.RED + 
						p.getLocation().getY() + ChatColor.GOLD + " Z: " + ChatColor.RED + p.getLocation().getZ();
