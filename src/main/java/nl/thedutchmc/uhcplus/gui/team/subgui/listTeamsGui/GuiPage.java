...
public class GuiPage {
	...
	void setupGui() {
		...
		// Gui size has to be a multiple of 9, so calculate the next multiple of 9 based
		// on the base size
		guiSize = baseSize % 9 == 0 ? baseSize : baseSize + (9 - (baseSize % 9));

		// Gui cant be larger than 54 slots (6 rows of 9), so we add a new page and set
		// gui size to 54
		if (guiSize > 54) {
			nextPage = true;
			guiSize = 54;
	...
	void setupItems() {
		...
		for (int i = (51 * pageId); i < TeamHandler.teams.size() && j < 51; i++, j++) {
			...
			gui.setItem(j, ListTeamsGui.createItems(Material.PAPER,
					ChatColor.RESET + "" + team.getTeamColor() + "Team " + i, lore));

		}

		// Calculate the position of the back button based on if there's a next page,
		// and add the item to the inventory
		int positionBackButton = nextPage ? guiSize - 3 : guiSize - 2;
		gui.setItem(positionBackButton,
				CreateItem.create(Material.WHITE_STAINED_GLASS_PANE, ChatColor.RESET + "Previous page"));
